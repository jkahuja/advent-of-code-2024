...
def examine_candidate_recursive(i, j, ch, i_change, j_change):
    ...
    if ch == "S" and words[i][j] == ch:
        return 1
    return examine_candidate_recursive(
        i + i_change, j + j_change, NEXT_CHAR_MAP[ch], i_change, j_change
    )


def examine_candidate(i, j, ch):
    return (
        examine_candidate_recursive(i - 1, j, ch, -1, 0)
        + examine_candidate_recursive(i + 1, j, ch, 1, 0)
        + examine_candidate_recursive(i, j - 1, ch, 0, -1)
        + examine_candidate_recursive(i, j + 1, ch, 0, 1)
        + examine_candidate_recursive(i - 1, j - 1, ch, -1, -1)
        + examine_candidate_recursive(i + 1, j + 1, ch, 1, 1)
        + examine_candidate_recursive(i + 1, j - 1, ch, 1, -1)
        + examine_candidate_recursive(i - 1, j + 1, ch, -1, 1)
    )
